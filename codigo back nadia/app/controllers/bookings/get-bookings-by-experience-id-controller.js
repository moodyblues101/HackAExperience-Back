...
const { findBookingsByExperienceId } = require("../../repositories/bookings-repository");
...
async function getBookingsByExperienceId(req, res) {
    try {
        ...
        if (bookings.length === 0) {
            throwJsonError(404, `No existen reservas para la experiencia con id ${experienceId}`);
        }
