...
async function uploadImageProfile(req, res) {
    try {
        ...
        if (!validExtensions.includes(extension)) {
            throwJsonError(400, 'Formato no válido, únicamente puede subir archivos con extensión .jpg, .jpeg o .png')
        }
        ...
        profileImage.mv(pathImage, async function (err) {
            ...
            res.status(201).send({ url: `${HTTP_SERVER}/${PATH_USER_IMAGE}/${imageName}` });
        })
    } catch (error) {
