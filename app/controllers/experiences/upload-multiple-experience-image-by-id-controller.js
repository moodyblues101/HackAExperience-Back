...
const { addImageByExperienceId } = require("../../repositories/experience-images-repository");
...
async function uploadMultipleExperienceImages(req, res) {
    try {
        ...
        const uploadImages = await Promise.all(imageExperience.map(async (imgExperience) => {
            ...
            return ({ image: `${HTTP_SERVER}/${PATH_EXPERIENCE_IMAGE}/${experienceId}/${processImage}` });
        }));
